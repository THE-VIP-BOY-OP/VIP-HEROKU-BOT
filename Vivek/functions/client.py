...
class VClient(Client):
    ...
    async def load_database(self):
        ...
        msg = max(
            messages, key=lambda msg: max(msg.date, msg.edit_date or msg.date)
        )
    ...
    async def export_database(self):
        ...
        if os.path.isfile(".mydatabase.db"):
            media = InputMediaDocument(media=".mydatabase.db", caption=f"this is DATABASE of {self.me.mention} Please don't Delete or Unpin This message\nElse your bot data will be deleted")
            await self.edit_message_media(chat_id=DATABASE_CHANNEL_ID, message_id=msg.id, media=media, file_name=".mydatabase.db")
