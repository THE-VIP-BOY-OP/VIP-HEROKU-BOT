...
from config import UPSTREAM_BRANCH, LOG_GROUP_ID, UPSTREAM_REPO, GIT_TOKEN
...
async def update_(client, message):
    ...
    if len(updates) > 4096:
        ...
        await response.edit(f"**A new update is available for the bot**\n\n[Check Updates]({url})", disable_web_page_preview=True)
    else:
