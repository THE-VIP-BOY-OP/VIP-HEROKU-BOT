...
async def export_database(client, message):
    ...
    for db_name in databases:
        ...
        if db_name != DB_NAME:
            mystic = await edit_or_reply(
                mystic, f"Found data of {db_name} database. Uploading and deleting..."
            )
            file_path = await ex_port(db, db_name)
            await app.send_document(
                message.chat.id, file_path, caption=f"MongoDB data of {db_name}"
            )
    ...
    await app.send_document(
        message.chat.id,
        file_path,
        caption=f"MongoDB backup of {app.mention}. You can import this into a new MongoDB instance by replying with /import",
        progress=progress,
    )
    await mystic.delete()
...
async def import_database(client, message):
    if not message.reply_to_message or not message.reply_to_message.document:
        return await message.reply_text(
            "You need to reply to an export file to import it."
        )
    ...
    try:
        ...
        return await edit_or_reply(
            mystic, "Invalid exported data. Please provide a valid MongoDB export."
        )

    if not isinstance(data, dict):
        return await edit_or_reply(
            mystic, "Invalid data format. Please provide a valid MongoDB export."
        )
