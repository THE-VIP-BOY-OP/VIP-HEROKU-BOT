...
async def get_common_chats(client, message):
    ...
    if len(message.command) < 2 and not message.reply_to_message:
        return await message.reply_text("Usage: /common_chats @username or reply to a user.")

    if message.reply_to_message:
        username = message.reply_to_message.from_user.username or message.reply_to_message.from_user.id
    else:
